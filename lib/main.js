...
exports.main =async  function(db,message,voiceText,createAudioPlayer,createAudioResource,AudioPlayerStatus,writeFileSync,getVoiceConnection,StreamType,joinVoiceChannel,dic,sp,createWriteStream,time){
  ...
  if(!message.member.voice.channel) return
   ...
   const cyannel = await db.get(`${message.guild.id}_messagechannel`)
   if(!cyannel || !(cyannel === message.channel.id)) return
   if(!voice_actor) voice_actor = "hikari"
    if (!v) return
      ...
      let txt = ""
/*
     let time_use_tts = time.get(`${message.guild.id}_time_speak`)
     if(!time_use_tts) time_use_tts = 0
     const save_time_use_tts = time_use_tts + message.content.length
     time.set(`${message.guild.id}_time_speak`,save_time_use_tts)
*/
